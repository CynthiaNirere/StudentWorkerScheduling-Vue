...
const todayTasks = computed(() => {
  ...
  return taskLists.value.filter(t => {
    ...
    return dueDate === today || t.recursDaily;
  });
});

const templateTasks = computed(() => {
  return taskLists.value.filter(t => t.isTemplate);
});
...
const yesterdayCompletions = computed(() => {
  ...
  return completionHistory.value.filter(h => {
    const completedDate = new Date(Number(h.completedAt));
...
const openItemsDialog = async (taskList) => {
  ...
  await loadTaskListItems(taskList.id);
};
...
const addTaskItem = async () => {
  ...
  try {
    await EmployerService.createTaskListItem({
      ...newItem.value,
      tasklistId: selectedTaskList.value.id,
      status: 'pending',
      createdAt: Date.now()
    });
    ...
    resetNewItem();
    await loadTaskListItems(selectedTaskList.value.id);
  } catch (error) {
...
const toggleItemCompletion = async (item) => {
  try {
    ...
    await EmployerService.updateTaskListItem(item.id, {
      status: newStatus,
      completedAt: newStatus === 'completed' ? Date.now() : null,
      completedBy: newStatus === 'completed' ? user.value.id : null
    });
...
<template>
  <!-- ✅ WRAPPED WITH EmployerLayout FOR SIDEBAR -->
  <EmployerLayout>
    <v-container fluid class="pa-6">
      ...
      <v-window v-model="tab">
        ...
        <v-window-item value="today">
          <v-row>
            <v-col v-for="task in todayTasks" :key="task.id" cols="12" md="6" lg="4">
              <v-card hover @click="openItemsDialog(task)" class="task-card">
                <v-card-title class="d-flex align-center">
                  ...
                  {{ task.title }}
                  <v-spacer />
                  <v-chip v-if="task.recursDaily" size="x-small" color="purple" variant="tonal">
        ...
        <v-window-item value="templates">
          <v-row>
            <v-col v-for="task in templateTasks" :key="task.id" cols="12" md="6" lg="4">
              <v-card hover @click="openItemsDialog(task)" class="task-card">
                <v-card-title class="d-flex align-center">
                  <v-icon color="purple" class="mr-2">mdi-content-save</v-icon>
                  {{ task.title }}
                  <v-spacer />
                  <v-btn icon size="small" variant="text" @click.stop="deleteTaskList(task.id)">
        ...
        <v-window-item value="all">
          <v-card>
            <v-table>
              ...
              <tbody>
                <tr v-for="task in taskLists" :key="task.id">
                  <td>
                    <strong>{{ task.title }}</strong>
                    <v-chip v-if="task.isTemplate" size="x-small" color="purple" variant="tonal" class="ml-2">
                      Template
                    </v-chip>
                  </td>
                  <td>
                  ...
                  <td>{{ task.shiftType?.replace('_', ' ') }}</td>
                  <td>
                  ...
                  <td>
                    ...
                    <v-btn icon size="small" variant="text" @click="deleteTaskList(task.id)">
                      <v-icon>mdi-delete</v-icon>
                    </v-btn>
                  </td>
                </tr>
            ...
            <v-list v-if="selectedTaskList.items && selectedTaskList.items.length > 0">
              ...
              >
                ...
                <template v-slot:append>
                  <v-chip v-if="item.completedAt" size="x-small" color="success" variant="tonal">
                    {{ formatDate(item.completedAt) }}
                  </v-chip>
                  <v-btn icon size="small" variant="text" @click="deleteTaskItem(item.id)">
